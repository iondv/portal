...
function FileAdapter(options) {
  ...
  if (!options.dir) {
    throw new Error('FileAdapter: не указана директория контента.');
  }
      ...
      return new Promise(function (resolve, reject) {
        fs.stat(fn, function (err, stats) {
          ...
          fs.readFile(fn, {encoding: 'utf-8'}, function (err, data) {
            if (err) {
              if (options.log) {
                options.log.warn('Не удалось загрузить файл ресурса ' + fn + '. Причина:');
  ...
  function loadDir(pth, prefix) {
    if (options.log) {
      options.log.info('Загружается контент из ' + pth);
    }
    return new Promise(function (resolve, reject) {
      try {
