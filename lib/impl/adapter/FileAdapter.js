...
function FileAdapter(options) {
  ...
  if (!options.dir) {
    throw new Error('FileAdapter: content directory not specified.');
  }
      ...
      return new Promise(function (resolve, reject) {
        fs.stat(fn, function (err, stats) {
          ...
          fs.readFile(fn, {encoding: 'utf-8'}, function (err, data) {
            if (err) {
              if (options.log) {
                options.log.warn('Failed to load resource file ' + fn + '. Reason:');
  ...
  function loadDir(pth, prefix) {
    if (options.log) {
      options.log.info('Loading content from ' + pth);
    }
    return new Promise(function (resolve, reject) {
      try {
